"""
dftk.commands.base — abstract base class for all dftk subcommands.
...
``add_arguments`` to attach the standard dftk read + output argument groups.
...
class BaseCommand(ABC):
    """Abstract base class for all dftk subcommands."""
    ...
    @abstractmethod
    def help(self) -> str:
        """One-line description shown in ``dftk --help``."""
    ...
    def add_io_arguments(self, parser: argparse.ArgumentParser) -> None:
        """Attach the standard dftk read + output argument groups to *parser*.
