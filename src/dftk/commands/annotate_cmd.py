"""
dftk.commands.annotate_cmd — dftk annotate subcommand.
...
metadata.  dftk uses this to carry provenance annotations (genome build,
source dataset, processing steps, etc.) through a pipeline.

Usage examples:

  # List all annotations on a file
  dftk annotate data.parquet

  # Set / update annotations (modifies the file in-place)
  dftk annotate data.parquet --set genome=hg38 --set source=gwas_2024

  # Get a single annotation
  dftk annotate data.parquet --get genome

  # Remove an annotation
  dftk annotate data.parquet --delete genome

  # Embed annotations when writing parquet from any dftk command
