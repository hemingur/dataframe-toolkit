"""
stattools.commands.scat_cmd — dfstat scat subcommand.
...
Basic scatter:
  ...
  dfstat scat data.tsv -x weight -y height

Colour by group:

  dfstat scat data.tsv -x x -y y -g group

Subplot grid by condition, colour by treatment:

  dfstat scat data.tsv -x x -y y --subgraphcol condition -g treatment

Fit OLS line:

  dfstat scat data.tsv -x x -y y --fit

Publication figure (Nature single column, PDF):

  dfstat scat data.tsv -x x -y y --size single --fontsize publication -f fig.pdf

Bubble chart (point size from column):

  dfstat scat data.tsv -x x -y y --sizecol area_col

Continuous colour mapping:

  dfstat scat data.tsv -x x -y y --colorcol score
"""
