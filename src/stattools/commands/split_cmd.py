"""
stattools.commands.split_cmd — dfstat split subcommand.
...
-----------------
  By default output files are named:  <prefix><group_key><suffix>
    ...
    dfstat split data.tsv -g country -p out/ -s .tsv
    # → out/Iceland.tsv, out/Norway.tsv, ...

  With --template, use {col} placeholders for the group column values:

    dfstat split data.tsv -g year month --template data_{year}_{month}.tsv
...
--------
  ...
  dfstat split data.tsv -g country year -p results/ -s .tsv
  dfstat split data.tsv -g category --template out/{category}.tsv
"""
