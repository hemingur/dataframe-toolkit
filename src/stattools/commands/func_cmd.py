"""
stattools.commands.func_cmd — dfstat func subcommand.
...
Cumulative sum of "value":
  ...
  dfstat func data.tsv -c value -t cumsum

Group mean broadcast (adds "value_mean" unless -d is given):

  dfstat func data.tsv -c value -g group -t mean

Quantile bins (quartiles):

  dfstat func data.tsv -c score -t qcut:4 -d score_quartile

Multiple transforms in one call:

  dfstat func data.tsv -c expr -t cumsum -d expr_cumsum \\
      | dfstat func - -c expr -t mean -g condition -d expr_grpmean
"""
