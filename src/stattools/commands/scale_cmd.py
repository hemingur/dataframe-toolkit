"""
stattools.commands.scale_cmd — dfstat scale subcommand.
...
Default (scale):
  ...
  dfstat scale data.tsv -c y                        z-score (mean shift, std scale)
  dfstat scale data.tsv -c y --shift min            shift so min=0, scale by std
  dfstat scale data.tsv -c y --shift min --scale max shift so min=0, scale so max=1
  dfstat scale data.tsv -c y --shift none --scale sum no shift, scale so sum=1
  dfstat scale data.tsv -c y --scale none           shift to zero mean, no scaling

Rank normalization (--rank):

  dfstat scale data.tsv -c y --rank                 normal scores (default)
  dfstat scale data.tsv -c y --rank --rankdist uniform  uniform [0, 1]

Regression detrending (--resid):

  dfstat scale data.tsv -c y --resid -f "y ~ x"
  dfstat scale data.tsv -c y --resid -f "y ~ x" --nointercept

Grouping:

  dfstat scale data.tsv -c y -g group               z-score within each group
  dfstat scale data.tsv -c y --rank -g group         rank within each group
