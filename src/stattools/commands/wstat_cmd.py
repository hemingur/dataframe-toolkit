"""
stattools.commands.wstat_cmd — dfstat wstat subcommand.
...
----------------------
  ...
  If a single weight column is given it is broadcast to all stat columns:
    ...
    dfstat wstat data.tsv -c value -w count
    dfstat wstat data.tsv -c x y -w wx wy
    dfstat wstat data.tsv -c x y -w w          # w used for both x and y
...
--------
  dfstat wstat data.tsv -c value -w count
  dfstat wstat data.tsv -c value -w count -g group
  dfstat wstat data.tsv -c x y -w w --confidencelevel 99
"""
