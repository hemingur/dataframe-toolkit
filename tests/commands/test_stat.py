...
from stattools.commands.stat_cmd import _compute_stats, _STAT_NAMES
...
def _row(result: pd.DataFrame, name: str) -> pd.Series:
    ...
    assert len(rows) == 1, f"Expected exactly one row with name={name!r}, got {len(rows)}"
    return rows.iloc[0]
...
class TestUngroupedSingleCol:
    ...
    def test_ci_99(self, simple_df, args_factory):
        """Custom confidence level is forwarded correctly."""
        result = _compute_stats(simple_df, args_factory(cols=["x"], confidencelevel=99.0))
        x = simple_df["x"].to_numpy()
...
class TestUngroupedMultiCol:
    ...
    def test_schema(self, simple_df, args_factory):
        result = _compute_stats(simple_df, args_factory(cols=["x", "y"]))
        assert list(result.columns) == _ALL_OUTPUT_COLS

    def test_two_rows(self, simple_df, args_factory):
        result = _compute_stats(simple_df, args_factory(cols=["x", "y"]))
        assert len(result) == 2

    def test_name_values(self, simple_df, args_factory):
        result = _compute_stats(simple_df, args_factory(cols=["x", "y"]))
        assert set(result["name"]) == {"x", "y"}

    def test_y_mean_is_10x_x(self, simple_df, args_factory):
...
class TestGroupedSingleCol:
    ...
    def test_schema(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        assert list(result.columns) == ["group", "name"] + _STAT_NAMES

    def test_two_rows(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        assert len(result) == 2  # one row per (group, col) combination

    def test_group_names(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        assert set(result["group"]) == {"A", "B"}

    def test_group_a_mean(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        row = _group_row(result, "group", "A", "value")
        assert row["mean"] == pytest.approx(2.0)

    def test_group_b_mean(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        row = _group_row(result, "group", "B", "value")
        assert row["mean"] == pytest.approx(20.0)

    def test_group_a_count(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        row = _group_row(result, "group", "A", "value")
        assert row["count"] == 3

    def test_group_b_sum(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        row = _group_row(result, "group", "B", "value")
        assert row["sum"] == pytest.approx(60.0)

    def test_group_a_ci(self, grouped_df, args_factory):
        result = _compute_stats(grouped_df, args_factory(cols=["value"], groupcol=["group"]))
        row = _group_row(result, "group", "A", "value")
    ...
    def test_kurt_present(self, grouped_df, args_factory):
...
class TestGroupedMultiCol:
    ...
    def test_schema(self, two_group_two_col_df, args_factory):
        result = _compute_stats(
            two_group_two_col_df,
            args_factory(cols=["x", "y"], groupcol=["group"]),
        )
        assert list(result.columns) == ["group", "name"] + _STAT_NAMES

    def test_four_rows(self, two_group_two_col_df, args_factory):
    ...
    def test_name_values(self, two_group_two_col_df, args_factory):
        result = _compute_stats(
            two_group_two_col_df,
            args_factory(cols=["x", "y"], groupcol=["group"]),
        )
    ...
    def test_group_a_x_mean(self, two_group_two_col_df, args_factory):
        result = _compute_stats(
            two_group_two_col_df,
            args_factory(cols=["x", "y"], groupcol=["group"]),
        )
    ...
    def test_group_b_y_mean(self, two_group_two_col_df, args_factory):
        result = _compute_stats(
            two_group_two_col_df,
            args_factory(cols=["x", "y"], groupcol=["group"]),
        )
...
class TestEdgeCases:
    ...
    def test_name_col_clash_warning(self, args_factory, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="stattools"):
            result = _compute_stats(df, args_factory(cols=["value"], groupcol=["name"]))
        assert "name_mangled" in result.columns
        assert "name" in caplog.text

    def test_ci_method_nearest(self, simple_df, args_factory):
        """confidencemethod is forwarded to np.percentile."""
        result = _compute_stats(
            simple_df,
            args_factory(cols=["x"], confidencemethod="nearest"),
        )
        x = simple_df["x"].to_numpy()
        expected_lo = np.percentile(x, 2.5, method="nearest")
        assert result["cilo"].iloc[0] == pytest.approx(expected_lo)

    def test_stat_names_order(self, simple_df, args_factory):
