...
def _run(df, **kwargs):
    ...
    defaults = dict(
        DATAFILE=fname,
        indexcols=None, destcol="variable", valuecol="value",
...
class TestMeltBasic:
    ...
    def test_values_are_correct(self, wide_df):
        rows = _run(wide_df, indexcols=["sample"])
        s1_a = next(r for r in rows if r["sample"] == "s1" and r["variable"] == "gene_A")
    ...
    def test_missing_indexcol_raises(self, wide_df):
        ...
        try:
            args = argparse.Namespace(
                DATAFILE=fname, indexcols=["no_such_col"],
                destcol="variable", valuecol="value",
                select=None, drop=None, move=None, na_rep=None, dropna=False,
                postquery=[], cast=None, sortasc=None, sortdesc=None, sort=None,
                expect=None, round=None, sigdig=None,
                movetofront=None, movetoback=None,
